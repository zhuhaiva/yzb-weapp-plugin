...
Component({
  ...
  data: {
    code: "",
    tuangouList: [],
    imageSrc: 'https://dzb.dzbsaas.com/upload/image/20250417/bfb9993f-142c-4ba3-b221-bf2957ccaf43-source.png',
  },
  observers: {
    mobile: function (val) {
      if (val) {
  ...
  methods: {
    ...
    getTuangouInfoHandler(mobile) {
      getTuangouInfo({}).then(res => {
        const {
          boundMeituanTuangouSets
        } = res.data      
        const list = []
        boundMeituanTuangouSets.forEach((item, index) => {
          this.queryMeituanProductByMobileHandler(item, mobile).then(evoucher => {
            list.push({
              ...item,
              evoucher
            })
    ...
    verifyMeituanHandler(code) {
      ...
      verifyMeituan({
        ...
        if (res.message === '成功') {
          ...
          const list = tuangouList.map(item => {
