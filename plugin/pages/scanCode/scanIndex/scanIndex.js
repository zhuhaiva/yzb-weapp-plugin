...
import { formatTime, getRequest } from "../../../utils/util";
import {
  getScanteatableAPI,
  getlockAPI,
  ringcallApi,
...
Page({
  ...
  onLoad: function (options) {
    const { id, teaTableId, time } = options;
    ...
    return new Promise((resolve) => {
      ...
      getScanteatableAPI(teaTableId).then((data) => {
        ...
        if (Object.keys(data).length > 0) {
          const { checkIn, handcards, room, teaTable, tenant, setting } = data;
  ...
  getGoodsPage() {
    ...
    const url = `plugin://yzb-plugin/goodsorder?roomName=${room.name}&roomId=${
      room.id
    }&teaTable=${teaTable}&selfOrder=${setting.allowSelfOrder}&checkInId=${
      checkIn ? checkIn.id : ""
    }&checkInSN=${
      checkIn ? checkIn.sn : ""
    }&tenantId=${tenantId}&lockcode=${currentLockCode}&isSCM=${isSCM}&isBusiness=${isBusiness}&openTime=${
      setting.openTime
    }&closeTime=${setting.closeTime}&islock=${
      currentLockCode != null
    }&limitBuyProduct=${setting.limitBuyProduct}`;
    wx.navigateTo({
      url,
    });
  },
  ...
  getPayMemtPage() {
    ...
    const { tenantId, room, currentLockCode, setting } = this.data;
    wx.navigateTo({
      url: `plugin://yzb-plugin/payment?roomId=${
        room.id
      }&tenantId=${tenantId}&checkInId=${checkIn.id}&islock=${
        currentLockCode != null
      }&deviceNo=${setting.deviceNo}&allowSelfPay=${setting.allowSelfPay}`,
    });
  },
  /**
   * 下拉刷新
  ...
  isBusinessFunc(options) {
    const { openTime, closeTime } = options;
